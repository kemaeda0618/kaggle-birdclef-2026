"""Generate exp102: exp017 R2 single teacher → l1 student (single fold) NB.
...
Run: python _gen_nb_train_r3.py
"""
import json
...
cells.append(md_cell(r"""# exp102 R3 (l1 single fold, exp017 R2 teacher)
...
- 最終: Kaggle Dataset `birdclef2026-exp102-l1-3fold-r3p` に R3 ckpt upload
...
DRIVE_EXP_DIR    = DRIVE_INPUT_DIR / "output" / "exp102"
...
# ★ exp102: teacher pseudo (pseudo_e17.csv) を Kaggle NB output から DL → Drive へ
...
class BirdSEDModel(nn.Module):
    def __init__(self, backbone_name=BACKBONE, num_classes=NUM_CLASSES,
                 drop_path_rate=0.15, hidden_dim=512):  # ★ exp102: 0.1 -> 0.15 (Babych R2+ spec)
        super().__init__()
...
class MixSamp(torch.utils.data.Sampler):
    # exp102: per-source mixing + per-class inverse-freq weighting for focal source.
    ...
    def __init__(self, sizes, names, shares, bs, nst, seed=0, focal_weights=None):
        ...
        # ★ exp102: focal_weights — must match focal source size, normalized to sum 1
        ...
        for _ in range(self.nst):
            ...
            for off, size, name, n in zip(self.offsets, self.sizes, self.names, self.per_src):
                ...
                if name == "focal" and self.focal_weights is not None:
                    # ★ exp102: per-class inverse-freq weighted sampling for focal
...
# ★ exp102: M7-style per-species + class stats + taxon line
...
for FOLD_K in FOLDS:
    print(f"\n{'#'*70}\n# exp102 R3 (l1 student, e17 teacher) Fold {FOLD_K} (of {N_FOLDS}-fold split, single fold training)\n{'#'*70}")
    DRIVE_FOLD_DIR  = DRIVE_EXP_DIR / f"fold{FOLD_K}"
    # ★ exp102: teacher pseudo は exp017 R2 (pseudo_e17.csv) を Drive 上に置く前提
...
USER  = "maekeso"
SLUG  = "birdclef2026-exp102-l1-3fold-r3p"
TITLE = "birdclef2026 exp102 l1 3fold r3p"  # 32 chars (Kaggle title limit 50)

with tempfile.TemporaryDirectory() as td:
    td = Path(td)
    n_staged = 0
    for fold_k in FOLDS:
        # ★ exp102: R3 ckpt のみ upload (R1/R2 は別 exp で管理)
    ...
    version_notes = f"exp102 l1 single (R3 from e17) | {summary}"[:498]
...
print(f"\n{'='*60}")
print(f"exp102 l1 single fold R3 summary")
print(f"{'='*60}")
...
for k, v in sorted(fold_results.items()):
    print(f"  Fold {k}: {v}")
print(f"\n  Kaggle Dataset: https://www.kaggle.com/datasets/maekeso/birdclef2026-exp102-l1-3fold-r3p")
""", "summary"))
