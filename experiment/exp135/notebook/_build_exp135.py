"""Build exp135: exp110 + amphibian v1 surgical blend at ALPHA=0.3 (conservative variant of exp130).

Identical to exp130 (v1 model amphib-b0-ov) except ALPHA_AMPH=0.3 (more base weight, FP-safer).
Lets us compare alpha=0.5 (exp130) vs alpha=0.3 (exp135) on real LB.
"""
import io, json, sys
...
DST = Path("experiment/exp135/notebook/nb_exp135_amphib_a03.ipynb")
DST.parent.mkdir(parents=True, exist_ok=True)
nb = json.loads(SRC.read_text(encoding="utf-8"))

AMPH_CELL = """# === exp135: amphibian specialist V1 (effb0, torch) inference — aligned with audio_cache ===
...
print("[exp135] AMPH_DIR:", _AMPH_DIR)
...
print(f"[exp135] amphib v1 inference done: {amph_df.shape}, labels={len(AMP_LABELS)}")
"""

ids = [c.get("id") for c in nb["cells"]]
assert "e29-infer" in ids, ids
nb["cells"].insert(ids.index("e29-infer") + 1,
    {"cell_type": "code", "id": "exp135_amphib", "metadata": {}, "execution_count": None, "outputs": [],
     "source": AMPH_CELL.splitlines(keepends=True)})

TOCSV = 'submission.to_csv("submission.csv", index=False)'
BLEND = '''# === exp135: amphibian V1 surgical rank-blend ALPHA=0.3 (35 cols only; 199 untouched) ===
...
for _j, _lbl in enumerate(AMP_LABELS):
    ...
    _nb += 1
assert np.array_equal(submission[_nonamp_cols].values, _pre_nonamp), "exp135: non-amphibian cols changed!"
print(f"[exp135] amphibian surgical blend: {_nb} cols (alpha={ALPHA_AMPH}); {len(_nonamp_cols)} non-amph unchanged")
''' + TOCSV
...
for c in chk["cells"]:
    if c.get("id") in ("exp135_amphib", "blend"):
