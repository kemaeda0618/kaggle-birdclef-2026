"""Generate exp028: exp027 ProtoSSM-only + Sonotype mirror (existing + Tier 1 new).

exp027 (NB4 ProtoSSM-only, W_PROTO=1.0, W_MLP=0.0) をベースに、提出 cell で
Sonotype mirror max-pool を適用。EXISTING_MIRROR_PAIRS (公開 0.948 NB 由来) に加え、
NEW_MIRROR_PAIRS_TIER1 (data-driven、J≥0.5 AND A≥0.85) を追加。

目的: ProtoSSM-only 単独軸でも sonotype mirror が +0.001-0.003 寄与するか測る。
exp026 (blend に mirror 追加版) と組合せて、mirror 効果が blend 内で吸収されてるか分離検証。

新規 Tier 1 ペア (data-driven、experiment/sonotype_viz 解析):
- (s06, s14)  J=0.667  A=0.947
- (s02, s06)  J=0.389  A=0.946  → s02-s14 と組合せて (s02, s06, s14) 形成
- (s08, s20)  J=0.706  A=0.943
- (s11, s24)  J=0.667  A=0.943

期待 LB:
- exp027 base 比 +0.001-0.003 (mirror が ProtoSSM-only でも単独で効く)
- exp026 blend 比 -0.005~+0.000 (blend 無し分やや低い)

Pipeline (exp027 と同じ + mirror):
1. Load pre-computed embeddings
2. Train ProtoSSM (5 seeds) on labeled soundscapes
3. Run Perch ONNX on test_soundscapes (CPU)
4. Inference: TTA x seeds -> ProtoSSM logits only -> retrieval -> FCS -> season prior
   -> ★ Sonotype mirror max-pool (existing + Tier 1) ★ -> submission.csv

Usage: python _gen_nb_protossm_mirror.py
"""
import json
...
cells.append(md_cell("hdr",
    "# exp028: ProtoSSM-only + Sonotype mirror (existing + Tier 1 new)\n"
    "\n"
    "CPU submission notebook (90 min limit). exp027 (W_PROTO=1.0 / W_MLP=0.0) をベースに、\n"
    "提出 cell で Sonotype mirror max-pool を適用。**新規 Tier 1 ペア (data-driven) を追加**:\n"
    "- (s06, s14)  J=0.667  A=0.947\n"
    "- (s02, s06)  J=0.389  A=0.946  → s02-s14 と合体して (s02, s06, s14) 形成\n"
    "- (s08, s20)  J=0.706  A=0.943\n"
    "- (s11, s24)  J=0.667  A=0.943\n"
    "\n"
    "目的: ProtoSSM-only 単独軸 (blend 無) で sonotype mirror が +0.001-0.003 寄与するか分離計測。\n"
    "\n"
    "Same shared infra (exp027 と同): Prior Tables, multi-seed=5, TTA, SC retrieval, TA retrieval,\n"
    "file_conf_scale, season prior, KD (LAMBDA_KD=0.15)。\n"
    "Mirror 適用位置: season prior の直後、submission.csv 化の直前。"))
...
# ★ exp028: exp027 (ProtoSSM only) + Sonotype mirror (existing + Tier 1 new)
# MIRROR_PAIRS は SUBMISSION cell で適用 (season prior 直後)
W_PROTO = 1.0
W_MLP = 0.0
...
for group in MIRROR_PAIRS:
    ...
    if len(valid_idx) >= 2:
        group_max = preds_array[:, valid_idx].max(axis=1, keepdims=True)
        preds_array[:, valid_idx] = group_max
        mirror_count += len(valid_idx)
print(f"Sonotype mirror applied: {len(MIRROR_PAIRS)} groups, {mirror_count} columns")
# === end Sonotype mirror ===
...
out_path = HERE / "nb_protossm_mirror.ipynb"
