"""Generate exp028 nb_pseudo_exp015: exp015 (convnext_pico R2 single fold) predictions on train_soundscapes (Kaggle GPU).
...
output: /kaggle/working/pseudo_exp015.csv

Run: python _gen_nb_pseudo_exp015.py  ->  writes nb_pseudo_exp015.ipynb
"""
import json
...
cells.append(md_cell(r"""# exp028 nb_pseudo_exp015: l0 R2 5-fold predictions on train_soundscapes (Kaggle GPU)
...
## Output
- `/kaggle/working/pseudo_exp015.csv` (filename, start_sec, end_sec, [234 species probs])
- ~10,658 files × 12 windows = ~128k rows × 234 species ≈ 100-200 MB CSV

## 後段
R3 学習 NB が pseudo_exp015 + pseudo_tucker + pseudo_exp015 の 3 つを平均 →
...
for p in CANDIDATES:
    if p.exists() and (p / "r2_ckpt_best_ns22.pth").exists():
        STATE_DIR = p; break
...
assert STATE_DIR is not None, (
    "ckpt not found. Attach maekeso/birdclef2026-exp015-weights as dataset_sources"
)
print(f"State dir: {STATE_DIR}")
ckpt_files = [STATE_DIR / "r2_ckpt_best_ns22.pth"]
...
cells.append(code_cell(r"""# ============================================================
# Cell 4: Model — rebuild convnext_pico.d1_in1k SED architecture
# ============================================================
...
# 5 model を VRAM に常駐 (convnext_pico.d1_in1k × 5 ≈ 500 MB、T4 16 GB 十分)。
...
cells.append(code_cell(r"""# ============================================================
# Cell 7: Write pseudo_exp015.csv (R3 学習の input になる teacher pseudo)
# ============================================================
...
out_path = Path("/kaggle/working/pseudo_exp015.csv")
df_pseudo.to_csv(out_path, index=False)
print(f"pseudo_exp015.csv: {len(df_pseudo)} rows, {df_pseudo.shape[1]-3} species cols, "
...
np.save("/kaggle/working/pseudo_exp015.npy", probs.astype(np.float16))
np.save("/kaggle/working/pseudo_exp015_meta.npy", np.array(list(zip(all_filenames, all_start_sec, all_end_sec)), dtype=object))
print(f"\nAlso saved: pseudo_exp015.npy (float16, {Path('/kaggle/working/pseudo_exp015.npy').stat().st_size/1e6:.1f}MB)")
print(f"            pseudo_exp015_meta.npy")
""", "submit"))
...
out_path = HERE / "nb_pseudo_exp015.ipynb"
