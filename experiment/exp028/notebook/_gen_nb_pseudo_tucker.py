"""Generate exp028 nb_pseudo_tucker: Tucker public SED 5-fold ONNX predictions on train_soundscapes (Kaggle GPU).
...
output: /kaggle/working/pseudo_tucker.csv

Run: python _gen_nb_pseudo_tucker.py  ->  writes nb_pseudo_tucker.ipynb
"""
import json
...
cells.append(md_cell(r"""# exp028 nb_pseudo_tucker: l0 R2 5-fold predictions on train_soundscapes (Kaggle GPU)
...
## Output
- `/kaggle/working/pseudo_tucker.csv` (filename, start_sec, end_sec, [234 species probs])
- ~10,658 files × 12 windows = ~128k rows × 234 species ≈ 100-200 MB CSV

## 後段
R3 学習 NB が pseudo_tucker + pseudo_tucker + pseudo_e17 の 3 つを平均 →
...
# Locate Tucker public SED 5-fold ONNX (sed_fold{0..4}.onnx)
STATE_DIR = None
CANDIDATES = [
...
for p in CANDIDATES:
    if p.exists() and (p / "sed_fold0.onnx").exists():
        STATE_DIR = p; break
...
assert STATE_DIR is not None, (
    "Tucker ONNX not found. Attach tuckerarrants/bc2026-distilled-sed-public as dataset_sources"
)
print(f"Tucker ONNX dir: {STATE_DIR}")
ckpt_files = sorted(STATE_DIR.glob("sed_fold*.onnx"),
                    key=lambda p: int(re.search(r"sed_fold(\d+)", p.name).group(1)))
print(f"Found {len(ckpt_files)} Tucker SED fold ONNX:")
for f in ckpt_files:
...
for ckpt_path in ckpt_files:
    fold_id = int(re.search(r"sed_fold(\d+)", ckpt_path.name).group(1))
...
print(f"\nOK loaded {len(tucker_sessions)} Tucker SED ONNX sessions")
""", "load_ckpts"))
...
cells.append(code_cell(r"""# ============================================================
# Cell 7: Write pseudo_tucker.csv (R3 学習の input になる teacher pseudo)
# ============================================================
...
out_path = Path("/kaggle/working/pseudo_tucker.csv")
df_pseudo.to_csv(out_path, index=False)
print(f"pseudo_tucker.csv: {len(df_pseudo)} rows, {df_pseudo.shape[1]-3} species cols, "
...
np.save("/kaggle/working/pseudo_tucker.npy", probs.astype(np.float16))
np.save("/kaggle/working/pseudo_tucker_meta.npy", np.array(list(zip(all_filenames, all_start_sec, all_end_sec)), dtype=object))
print(f"\nAlso saved: pseudo_tucker.npy (float16, {Path('/kaggle/working/pseudo_tucker.npy').stat().st_size/1e6:.1f}MB)")
print(f"            pseudo_tucker_meta.npy")
""", "submit"))
...
out_path = HERE / "nb_pseudo_tucker.ipynb"
