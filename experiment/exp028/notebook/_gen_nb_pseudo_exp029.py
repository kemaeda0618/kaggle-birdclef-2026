"""Generate exp028 nb_pseudo_exp029: e17 (exp017 R2 single fold) predictions on train_soundscapes (Kaggle GPU).

目的: Multi-teacher R3 pseudo の teacher 3 として、exp017 R2 single fold の
train_soundscapes 全件 (~10,658 files) 予測を生成・保存する。

Single fold model なので l0r2 (5-fold) の 1/5 時間 (~30-60 min on T4 GPU)。
output: /kaggle/working/pseudo_exp029.csv

Run: python _gen_nb_pseudo_exp029.py  ->  writes nb_pseudo_exp029.ipynb
"""
import json
...
cells.append(md_cell(r"""# exp028 nb_pseudo_exp029: l0 R2 5-fold predictions on train_soundscapes (Kaggle GPU)
...
## Output
- `/kaggle/working/pseudo_exp029.csv` (filename, start_sec, end_sec, [234 species probs])
- ~10,658 files × 12 windows = ~128k rows × 234 species ≈ 100-200 MB CSV

## 後段
R3 学習 NB が pseudo_exp029 + pseudo_tucker + pseudo_exp029 の 3 つを平均 →
...
# Locate exp017 R2 single fold ckpt (r2_ckpt_best_ns22.pth)
STATE_DIR = None
CANDIDATES = [
    Path("/kaggle/input/datasets/maekeso/birdclef2026-exp029-l1-single"),
    Path("/kaggle/input/birdclef2026-exp029-l1-single"),
]
CKPT_NAME = "r3_fold0_ckpt_best_ns22.pth"
for p in CANDIDATES:
    if p.exists() and (p / CKPT_NAME).exists():
        STATE_DIR = p; break

if STATE_DIR is None:
    for hit in Path("/kaggle/input").rglob(CKPT_NAME):
        STATE_DIR = hit.parent; break

assert STATE_DIR is not None, (
    f"ckpt not found ({CKPT_NAME}). Attach maekeso/birdclef2026-exp029-l1-single as dataset_sources"
)
print(f"State dir: {STATE_DIR}")
ckpt_files = [STATE_DIR / CKPT_NAME]
print(f"Found {len(ckpt_files)} ckpt (single fold):")
for f in ckpt_files:
...
# 5 model を VRAM に常駐 (eca_nfnet_l1 × 5 ≈ 500 MB、T4 16 GB 十分)。
...
print(f"\nOK loaded {len(fold_models)} model(s) on {device} (e17 single fold)")
print(f"  Total params: {sum(p.numel() for p in fold_models[0].parameters())/1e6:.1f}M")
""", "load_ckpts"))
...
cells.append(code_cell(r"""# ============================================================
# Cell 7: Write pseudo_exp029.csv (R3 学習の input になる teacher pseudo)
# ============================================================
...
out_path = Path("/kaggle/working/pseudo_exp029.csv")
df_pseudo.to_csv(out_path, index=False)
print(f"pseudo_exp029.csv: {len(df_pseudo)} rows, {df_pseudo.shape[1]-3} species cols, "
...
np.save("/kaggle/working/pseudo_exp029.npy", probs.astype(np.float16))
np.save("/kaggle/working/pseudo_exp029_meta.npy", np.array(list(zip(all_filenames, all_start_sec, all_end_sec)), dtype=object))
print(f"\nAlso saved: pseudo_exp029.npy (float16, {Path('/kaggle/working/pseudo_exp029.npy').stat().st_size/1e6:.1f}MB)")
print(f"            pseudo_exp029_meta.npy")
""", "submit"))
...
out_path = HERE / "nb_pseudo_exp029.ipynb"
