"""Generate exp028 nb_pseudo_e17: e17 (exp017 R2 single fold) predictions on train_soundscapes (Kaggle GPU).
...
output: /kaggle/working/pseudo_e17.csv

Run: python _gen_nb_pseudo_e17.py  ->  writes nb_pseudo_e17.ipynb
"""
import json
...
cells.append(md_cell(r"""# exp028 nb_pseudo_e17: l0 R2 5-fold predictions on train_soundscapes (Kaggle GPU)
...
## Output
- `/kaggle/working/pseudo_e17.csv` (filename, start_sec, end_sec, [234 species probs])
- ~10,658 files × 12 windows = ~128k rows × 234 species ≈ 100-200 MB CSV

## 後段
R3 学習 NB が pseudo_e17 + pseudo_tucker + pseudo_e17 の 3 つを平均 →
...
cells.append(code_cell(r"""# ============================================================
# Cell 7: Write pseudo_e17.csv (R3 学習の input になる teacher pseudo)
# ============================================================
...
out_path = Path("/kaggle/working/pseudo_e17.csv")
df_pseudo.to_csv(out_path, index=False)
print(f"pseudo_e17.csv: {len(df_pseudo)} rows, {df_pseudo.shape[1]-3} species cols, "
...
np.save("/kaggle/working/pseudo_e17.npy", probs.astype(np.float16))
np.save("/kaggle/working/pseudo_e17_meta.npy", np.array(list(zip(all_filenames, all_start_sec, all_end_sec)), dtype=object))
print(f"\nAlso saved: pseudo_e17.npy (float16, {Path('/kaggle/working/pseudo_e17.npy').stat().st_size/1e6:.1f}MB)")
print(f"            pseudo_e17_meta.npy")
""", "submit"))
...
out_path = HERE / "nb_pseudo_e17.ipynb"
