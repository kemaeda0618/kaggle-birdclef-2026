...
cells.append(md_cell(r"""# exp095 R3 (l0 + R2 exact spec, exp017 R2 teacher) — l0 で val→LB gap 改善狙い
...
- 最終: Kaggle Dataset `birdclef2026-exp095-l0-r2spec` に R3 ckpt upload
...
DRIVE_EXP_DIR    = DRIVE_INPUT_DIR / "output" / "exp095"
...
# ★ exp095 (= exp029 same): teacher pseudo (pseudo_e17.csv) を Kaggle NB output から DL → Drive へ
...
N_FOLDS = 5               # ★ exp095 (= exp029 same): 5-fold split で分割するが、train するのは fold 0 のみ
...
N_TOTAL_EPOCHS = 20         # ★ exp095 (= exp029 Option C): 15 → 20、warmup 4/20 = 20% で適正
BATCH = 192
LR = 3e-4
MIN_LR = 1e-6
WD = 1e-4
...
for FOLD_K in FOLDS:
    print(f"\n{'#'*70}\n# exp095 R3 (l0 student + R2 exact spec, e17 teacher) Fold {FOLD_K} (of {N_FOLDS}-fold split, single fold training)\n{'#'*70}")
    DRIVE_FOLD_DIR  = DRIVE_EXP_DIR / f"fold{FOLD_K}"
    # ★ exp095 (= exp029 same): teacher pseudo は exp017 R2 (pseudo_e17.csv) を Drive 上に置く前提
...
TITLE = "birdclef2026 exp095 l0 r2spec (R3 from e17)"

with tempfile.TemporaryDirectory() as td:
    td = Path(td)
    n_staged = 0
    for fold_k in FOLDS:
        # ★ exp095 (= exp029 same): R3 ckpt のみ upload (R1/R2 は別 exp で管理)
    ...
    version_notes = f"exp095 l0 R3 (R2 exact spec, e17 teacher) | {summary}"[:498]
...
print(f"\n{'='*60}")
print(f"exp095 l0 R3 summary (R2 exact spec)")
print(f"{'='*60}")
...
for k, v in sorted(fold_results.items()):
    print(f"  Fold {k}: {v}")
print(f"\n  Kaggle Dataset: https://www.kaggle.com/datasets/maekeso/birdclef2026-exp095-l0-r2spec")
""", "summary"))
