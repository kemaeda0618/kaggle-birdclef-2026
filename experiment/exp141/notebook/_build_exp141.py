"""Build exp141: exp110 + amphibian V5 surgical rank-blend (35 amphibian cols, alpha=0.5).

v5 = Pantanal-focused specialist (labeled-SC + strong AnuraSet), fair held-out col-AUC 0.798 (vs v1 0.655).
Same surgical design as exp130 (199 non-amphibian cols bit-identical, assert-verified) but v5 model.
"""
import io, json, sys
...
DST = Path("experiment/exp141/notebook/nb_exp141_amphib_v5_surgical.ipynb")
DST.parent.mkdir(parents=True, exist_ok=True)
nb = json.loads(SRC.read_text(encoding="utf-8"))

AMPH_CELL = """# === exp141: amphibian specialist V5 (effb0, torch) inference ===
import json as _json, timm as _timm
_AMPH_DIR = next((Path(p) for p in [
    "/kaggle/input/birdclef2026-amphib-b0-v5",
    "/kaggle/input/datasets/maekeso/birdclef2026-amphib-b0-v5"] if Path(p).exists()), None)
print("[exp141] AMPH_DIR:", _AMPH_DIR)
_amph_meta = _json.load(open(_AMPH_DIR / "amphib_v5_meta.json"))
...
_amph_model.load_state_dict(torch.load(str(_AMPH_DIR / "amphib_b0_v5.pth"), map_location="cpu"))
_amph_model.eval()
...
print(f"[exp141] amphib v5 inference done: {amph_df.shape}, labels={len(AMP_LABELS)}")
"""

ids = [c.get("id") for c in nb["cells"]]
assert "e29-infer" in ids, ids
nb["cells"].insert(ids.index("e29-infer") + 1,
    {"cell_type": "code", "id": "exp141_amphib", "metadata": {}, "execution_count": None, "outputs": [],
     "source": AMPH_CELL.splitlines(keepends=True)})

TOCSV = 'submission.to_csv("submission.csv", index=False)'
BLEND = '''# === exp141: amphibian V5 surgical rank-blend (35 cols; 199 untouched) ===
...
for _j, _lbl in enumerate(AMP_LABELS):
    ...
    _nb += 1
assert np.array_equal(submission[_nonamp_cols].values, _pre_nonamp), "exp141: non-amphibian cols changed!"
print(f"[exp141] amphibian V5 surgical blend: {_nb} cols (alpha={ALPHA_AMPH}); {len(_nonamp_cols)} non-amph unchanged")
''' + TOCSV
...
for c in chk["cells"]:
    if c.get("id") in ("exp141_amphib", "blend"):
