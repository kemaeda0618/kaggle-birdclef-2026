"""Generate exp010 NB4: ProtoSSM + MLP Head BLEND Submission Notebook.

Combines NB3 v29 (ProtoSSM, LB 0.921) + NB2 v10 (MLP Head, LB 0.918) in 1 NB,
blended in logit space. Both models share the same NB3 v29 infra:
- multi-seed=5 ensemble, TTA shifts [-1, 0, 1]
- site/hour metadata embedding, Prior Tables (lambda=0.3)
- Hierarchical Site-Conditioned KNN retrieval (lambda=0.10) applied once after blend
- file_confidence_scale (top_k=2, power=0.4) applied to final probs

Pipeline:
1. Load pre-computed embeddings
2. Train ProtoSSM (5 seeds) + MLP Head (5 seeds) on labeled soundscapes (66 files)
3. Run Perch ONNX on test_soundscapes (CPU)
4. Inference: TTA x seeds for both models -> logit blend -> retrieval -> file_conf_scale -> submission.csv

Usage: python _gen_nb4_blend.py
"""
import json
...
cells.append(md_cell("hdr",
    "# exp010 NB4 v6: ProtoSSM + MLP Head BLEND + Train Audio KNN Retrieval\n"
    "\n"
    "CPU submission notebook (90 min limit). Logit-space blend of two heads:\n"
    "- ProtoSSM (NB3 v29 LB 0.921): 12-window SSM seq processing + cross-attn + prototypes\n"
    "- MLP Head (NB2 v10 LB 0.918): per-window MLP + Gated Fusion\n"
    "\n"
...
D_MODEL = 256   # exp024 v1: ProtoSSM v5 upgrade (128→256)
D_STATE = 16
N_SSM_LAYERS = 3   # exp024 v1: ProtoSSM v5 upgrade (2→3)
