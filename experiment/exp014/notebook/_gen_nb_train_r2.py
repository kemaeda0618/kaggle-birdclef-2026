"""Generate exp014 R2 NB: HGNetV2-B0 SED + Perch distill + R1 pseudo (Babych H1).

Round 2 of Multi-Iterative Noisy Student (BC2025 1st-place Babych recipe):
- R1 student (exp014 baseline, LB 0.897) → pseudo on train_soundscapes
- R2 student trains on focal + labeled_sc + pseudo_sc
- Same architecture (HGNetV2-B0 + Perch v2 distill) as R1

Differences from R1:
- Source mix: focal 0.70 / labeled_sc 0.10 / pseudo_sc 0.20
- pseudo_sc source weight 0.5 (noisier than hard labels)
- Pseudo CSV loaded from `maekeso/exp014-r1-pseudo`
- Resume state: `maekeso/exp014-state-r2` (separate from R1 state)

Run: python _gen_nb_train_r2.py  ->  writes nb_train_r2.ipynb
"""
import json
...
# Cell 0: Header
# =============================================================================
cells.append(md_cell(r"""# exp014 R2 — Self-resuming HGNetV2-B0 SED + R1 pseudo (Kaggle T4x2)

**Round 2 of Multi-Iterative Noisy Student (Babych BC2025 1st-place recipe).**

R1 student (exp014, LB 0.897) generated pseudo labels on train_soundscapes.
This NB trains R2 student on:
- **focal**: BC2026 train_audio (35% share, weight 1.0)
- **labeled_sc**: BC2026 train_soundscapes_labels (10% share, weight 1.0)
- **pseudo_sc**: R1 pseudo on all 10,658 train_soundscapes (55% share — wait, see below)

**Wait — actually source mix is:** focal 0.70 / labeled_sc 0.10 / pseudo_sc 0.20

## How self-resume works (same pattern as R1)
1. First run: `maekeso/exp014-state-r2` doesn't exist → start fresh
2. End of each run: save full state to `ckpt_latest.pth`, upload to `maekeso/exp014-state-r2`
3. Next Save & Run All: dataset attaches, `resume_or_init` finds latest, continues
4. Trains 25 epochs total across multiple sessions
...
| `tuckerarrants/perch-v2-no-dft-onnx` | Perch v2 ONNX (Google) | Required | Required |
| `maekeso/exp014-r1-pseudo` | R1 pseudo CSV | Required | Required |
| `maekeso/exp014-state-r2` | Resume state | Skip (doesn't exist) | Required |
...
# =============================================================================
cells.append(code_cell(r"""# ============================================================
# Cell 2: Paths — locate competition data, Perch ONNX, pseudo CSV, resume state
# ============================================================
...
# --- Speed-up A: pre-copy train_soundscapes to local SSD (/kaggle/working) ---
# FUSE mount has ~50-200 ms per-file open() overhead; local SSD is ~1 ms.
# Cost: ~3-5 min one-time per session. Wipe-out between sessions (working dir).
...
if _n_src > 0 and _n_existing < _n_src:
    ...
    for _f in TS_DIR_SRC.glob("*.ogg"):
        ...
        if not _dst.exists():
            _shutil.copy(str(_f), str(_dst))
            _copied += 1
    _dt = _time.time() - _t0
    _n_final = sum(1 for _ in TS_DIR_LOCAL.glob("*.ogg"))
    print(f"  Copied {_copied} new files in {_dt:.1f}s  (total {_n_final})")
TS_DIR = TS_DIR_LOCAL  # override → all Dataset/path lookups use local SSD path
...
PSEUDO_CANDIDATES = [
    Path("/kaggle/input/datasets/maekeso/birdclef2026-exp014-pseudo-r1"),
    Path("/kaggle/input/birdclef2026-exp014-pseudo-r1"),
    Path("/kaggle/input/datasets/maekeso/exp014-r1-pseudo"),
    Path("/kaggle/input/exp014-r1-pseudo"),
]
...
assert PSEUDO_CSV_PATH is not None, (
    "pseudo_labels.csv not found. Attach maekeso/birdclef2026-exp014-pseudo-r1 as dataset_sources"
)
print(f"Pseudo CSV: {PSEUDO_CSV_PATH}")

# Resume state — check (a) state dataset, (b) own kernel output (kernel_sources)
# Note: dataset_create_version requires write-token; KGAT_ token gets 403 on upload.
# Fallback: read ckpt_latest.pth directly from this kernel's previous output.
RESUME_DIR = None
RESUME_CANDIDATES = [
...
for p in RESUME_CANDIDATES:
    if p.exists() and (p / "ckpt_latest.pth").exists():
        RESUME_DIR = p; break
    # also accept ckpt under subdir (rglob)
    if p.exists():
        hits = list(p.rglob("ckpt_latest.pth"))
        if hits:
            RESUME_DIR = hits[0].parent; break
print(f"Resume dir: {RESUME_DIR if RESUME_DIR else '(none, fresh R2 start)'}")
...
# =============================================================================
cells.append(code_cell(r"""# ============================================================
# Cell 3: Config — hyperparameters (R2 source mix differs from R1)
# ============================================================
NUM_CLASSES = 234
SR = 32000
...
# Source mixing — R2: add pseudo_sc as 3rd source
...
# =============================================================================
# Cell 4: Load data (focal + labeled SS + pseudo CSV)
# =============================================================================
cells.append(code_cell(r"""# ============================================================
# Cell 4: Load data — train.csv, taxonomy, labeled SS, pseudo CSV, fold assignment
# ============================================================
...
# --- Pseudo CSV (R1 output, soft labels on all train_soundscapes) ---
...
class BirdSEDModel(nn.Module):
    '''SED model: HGNetV2-B0 backbone + GeMFreq + 512-d bottleneck + attention head
...
# =============================================================================
cells.append(code_cell(r"""# ============================================================
# Cell 6: Datasets — FocalDS, LabeledSCDS, PseudoScDS, augmentation, samplers
# ============================================================
import soundfile as sf
import librosa
from functools import lru_cache

# Speed-up C: per-worker LRU cache for full 60s soundscape waveforms.
# Soundscape (TS) files are accessed at 12 different start_sec offsets per file.
# Decoding once + slicing avoids the per-window FUSE+decode cost.
# Each entry ~60s * 32kHz * 4B = 7.7 MB; maxsize=128 → ~1 GB/worker.
# Workers don't share cache; that's fine — opportunistic hits within a worker still pay off.
...
def _load_ogg(path, n_samples_target, start_sec=None):
    '''Load ogg with optional seek. Returns float32 numpy of length n_samples_target.'''
    if start_sec is not None:
        ...
        # tail short: pad with zeros
        out = np.zeros(n_samples_target, dtype=np.float32)
        avail = full[start:start + n_samples_target]
        out[:len(avail)] = avail
        return out
    # focal: variable-length, no offset → bypass cache (each file used once per epoch)
    try:
...
class LabeledSCDS(Dataset):
    '''Labeled soundscape window dataset.'''
...
class PseudoScDS(Dataset):
    '''Pseudo-labeled soundscape windows (R1 pseudo CSV). Soft labels.'''
...
# =============================================================================
# Cell 8: Build active datasets (per-fold) — adds pseudo_sc
# =============================================================================
cells.append(code_cell(r"""# ============================================================
# Cell 8: build_active_datasets per fold (R2: 3 sources)
# ============================================================
def build_active_datasets(fold_k):
    ...
    # Pseudo SC (not fold-filtered — unlabeled source, no leakage)
...
if RESUME_DIR is not None:
    ...
    print("No exp014-state-r2 dataset attached, starting fresh R2")
...
# =============================================================================
# Cell 12: Upload state to Kaggle Dataset (R2-specific dataset)
# =============================================================================
cells.append(code_cell(r"""# ============================================================
# Cell 12: Upload state to maekeso/exp014-state-r2
# ============================================================
...
DATASET_SLUG = "exp014-state-r2"
DATASET_TITLE = "exp014 R2 training state"
...
with tempfile.TemporaryDirectory() as td:
    ...
    version_notes = f"R2 epoch {trained_up_to}/{N_TOTAL_EPOCHS}"
    uploaded = False
    try:
...
out_path = HERE / "nb_train_r2.ipynb"
