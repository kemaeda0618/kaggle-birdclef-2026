"""Push exp014 R2 nb_train_r2.ipynb to Kaggle (T4x2 GPU, internet ON, self-resume).

Handles two graceful-fallback datasets:
- maekeso/exp014-r1-pseudo: REQUIRED for R2 training, but won't exist before
...
- maekeso/exp014-state-r2: gracefully missing on first run (just like R1).

Usage:  python _push_nb_train_r2.py

Slug rule (title slugifies to slug):
  title = "birdclef2026 exp014 train r2"
...
R2_STATE   = f"{USER}/exp014-state-r2"
...
for ds_sources, ks_sources in ATTEMPTS:
    try:
        ...
        last_err = msg
        if "not found" in msg.lower() or "404" in msg or "exp014-" in msg:
