"""Generate exp014 R2 inference NB.

Loads ckpt_best_ns22.pth from maekeso/birdclef2026-exp014-train-r2 (R2 train output),
rebuilds model, runs inference on test_soundscapes, writes submission.csv.
...
cells.append(md_cell(r"""# exp014 R2 — Inference (CPU, internet=off)

Load best R2 HGNetV2-B0 SED checkpoint from `maekeso/birdclef2026-exp014-train-r2`,
run inference on test_soundscapes (700 files * 12 windows), write submission.csv.
...
- kernel_sources: `maekeso/birdclef2026-exp014-train-r2` (R2 train output)

## Pipeline
1. Load `ckpt_best_ns22.pth` from R2 train output
...
assert STATE_DIR is not None, (
    "ckpt not found. Attach maekeso/birdclef2026-exp014-train-r2 as kernel_sources, "
    "or maekeso/exp014-state-r2 as dataset_sources"
)
print(f"State dir: {STATE_DIR}")
for f in sorted(STATE_DIR.rglob("ckpt_*.pth"))[:8]:
