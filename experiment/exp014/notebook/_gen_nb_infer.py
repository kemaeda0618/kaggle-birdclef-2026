"""Generate exp014 inference NB.

Loads ckpt_best_ns22.pth from maekeso/exp014-state, rebuilds model, runs inference
on test_soundscapes (or fallback to train_soundscapes for local debug), writes submission.csv.

The distill head is unused at inference (just SED clip + frame_max blend).
"""
import json
...
cells.append(md_cell(r"""# exp014 — Inference (CPU, internet=off)

Load best HGNetV2-B0 SED checkpoint from `maekeso/exp014-state`, run inference on
test_soundscapes (700 files * 12 windows), write submission.csv.
...
## Inputs
- `birdclef-2026` (competition data, includes test_soundscapes at score time)
- `maekeso/exp014-state` (training ckpts)

## Pipeline
1. Load `ckpt_best_ns22.pth`
...
5. Gaussian smooth across windows in logit space (Tucker-style post-proc)
...
# Resume / state — try kernel_sources (train NB output) first, fallback to dataset
STATE_DIR = None
CANDIDATES = [
    # kernel_sources (train NB output) — primary path
    Path("/kaggle/input/notebooks/maekeso/birdclef2026-exp014-train"),
    Path("/kaggle/input/birdclef2026-exp014-train"),
    # dataset_sources fallback
    Path("/kaggle/input/datasets/maekeso/exp014-state"),
    Path("/kaggle/input/exp014-state"),
]
for p in CANDIDATES:
    if p.exists():
        # Look for ckpt files (may be at root or under working/)
...
assert STATE_DIR is not None, (
    "ckpt not found. Attach maekeso/birdclef2026-exp014-train as kernel_sources, "
    "or maekeso/exp014-state as dataset_sources"
)
print(f"State dir: {STATE_DIR}")
for f in sorted(STATE_DIR.rglob("ckpt_*.pth")):
...
# Load best ckpt (rglob since path may be nested under working/)
...
# ============================================================
# Apply a soft per-file confidence boost so files with at least one strong
# detection get its other windows slightly amplified too. Matches Tucker recipe.
# Disabled by default; set ENABLE_FCS=True to use.
ENABLE_FCS = False
if ENABLE_FCS and probs.shape[0] > 0:
    ...
    # confidence scale: per-file max prob per species
    file_max = probs_3d.max(axis=1, keepdims=True)   # (n_files, 1, 234)
