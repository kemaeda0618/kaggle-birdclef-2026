"""Push exp136 FP-aware gate (labeled-SC standalone column AUC for v1+v3)."""
...
NB = Path(__file__).with_name("nb_exp136_fp_gate.ipynb")
USER = "maekeso"; SLUG = "birdclef2026-exp136-fp-gate"
with tempfile.TemporaryDirectory() as td:
    td = Path(td); shutil.copy(NB, td/NB.name)
    meta = {
        "id": f"{USER}/{SLUG}", "title": "birdclef2026 exp136 fp gate",
        "code_file": NB.name, "language": "python", "kernel_type": "notebook",
        "is_private": True, "enable_internet": True,
        "competition_sources": ["birdclef-2026"],
        "dataset_sources": ["maekeso/birdclef2026-amphib-b0-ov", "maekeso/birdclef2026-amphib-b0-v3"],
        "kernel_sources": [],
    }
