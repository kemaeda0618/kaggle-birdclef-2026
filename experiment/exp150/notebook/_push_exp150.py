"""Push exp150 diagnostic NB (per-stream x per-taxon AUC on labeled SC). Not a competition submission."""
...
NB = Path(__file__).with_name("nb_exp150_diag_perstream_taxon.ipynb")
USER = "maekeso"; SLUG = "birdclef2026-exp150-diag"
with tempfile.TemporaryDirectory() as td:
    td = Path(td); shutil.copy(NB, td/NB.name)
    meta = {
        "id": f"{USER}/{SLUG}",
        "title": "birdclef2026 exp150 diag",
        "code_file": NB.name,
