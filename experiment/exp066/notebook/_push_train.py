"""Push exp066 train NB to Kaggle (so user can pull in Colab).

Kaggle kernel に push して、Colab で kaggle CLI 経由で download 可能に。
"""
import json, os, sys, io, tempfile, shutil
from pathlib import Path
if os.name == "nt" and not os.environ.get("PYTHONUTF8"):
    os.environ["PYTHONUTF8"] = "1"; os.execv(sys.executable, [sys.executable] + sys.argv)
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding="utf-8")
if not os.environ.get("KAGGLE_API_TOKEN"):
    os.environ["KAGGLE_API_TOKEN"] = json.loads((Path.home()/".kaggle/kaggle.json").read_text())["key"]
from kaggle.api.kaggle_api_extended import KaggleApi
api = KaggleApi(); api.authenticate()

NB = Path(__file__).with_name("nb_train.ipynb")
USER = "maekeso"
SLUG = "birdclef2026-exp066-train"
TITLE = "birdclef2026 exp066 train"

with tempfile.TemporaryDirectory() as td:
    td = Path(td); shutil.copy(NB, td / NB.name)
    meta = {
        "id": f"{USER}/{SLUG}", "title": TITLE,
        "code_file": NB.name, "language": "python", "kernel_type": "notebook",
        "is_private": True,
        "enable_internet": True,  # Colab 用、競技 data 必要なし
        "competition_sources": [],
        "dataset_sources": [],
        "kernel_sources": [],
    }
    (td / "kernel-metadata.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
    r = api.kernels_push(str(td))
    print(f"URL: {getattr(r, 'url', None)}")
    print(f"Version: {getattr(r, 'version_number', None)}")
    err = getattr(r, "error", None)
    if err: print(f"Error: {err}")
print("\\n=== Download in Colab ===")
print(f"!kaggle kernels pull {USER}/{SLUG} -p .")
print("# → nb_train.ipynb (latest) Drive にダウンロード")
