"""Generate exp016 inference NB (Kaggle CPU 90min, internet=off).

Loads ckpt_best_ns22.pth from Kaggle Dataset `maekeso/birdclef2026-exp016-weights`
(uploaded separately via _push_weights_to_kaggle.py), rebuilds regnety_008 SED model,
...
cells.append(md_cell(r"""# exp016 — Inference (Kaggle CPU 90min, internet=off)

Load best regnety_008 SED ckpt from Kaggle Dataset, run inference on test_soundscapes,
...
- `maekeso/birdclef2026-exp016-weights` (R1 ckpts; upload via `_push_weights_to_kaggle.py`)
...
CKPT_PRIORITY = [
    "r2_ckpt_best_ns22.pth",   # R2 best (val 0.9265)
    "r2_ckpt_best_macro.pth",
    "r2_ckpt_latest.pth",
    "ckpt_best_ns22.pth",      # R1 fallback (val 0.9207)
    "ckpt_best_macro.pth",
    "ckpt_latest.pth",
]
ckpt_path = None
