"""Generate exp016 R2 NB: regnety_008 SED + R1 pseudo (Colab Pro A100).

Round 2 of Multi-Iterative Noisy Student:
- R1 student (exp016 regnety_008、val_ns22 0.9207) が生成した pseudo を 3rd source として使用
- R2 student も同 backbone (regnety_008)、新規初期化 (Babych 流)
...
入出力:
- 入力: Drive `output/exp016/r1-pseudo/pseudo_labels.csv` (R1 pseudo)
        Drive train_audio / train_soundscapes (Kaggle API 直 DL 経由)
- 出力 ckpt: Drive `output/exp016/r2/`
- 出力 pseudo: Drive `output/exp016/r2-pseudo/pseudo_labels.csv`
- Kaggle Dataset: `maekeso/birdclef2026-exp016-weights` (version up、R2 重み追加)
...
# Header
# =============================================================================
cells.append(md_cell(r"""# exp016 R2 — regnety_008 SED + R1 pseudo (Colab Pro A100)

**Round 2 of Multi-Iterative Noisy Student** (Babych BC2025 1位レシピ準拠)。

## What's new in R2
- R1 student (val_ns22 0.9207) が生成した pseudo CSV を 3rd training source として使用
- Source mix: **focal 0.70 / labeled_sc 0.10 / pseudo_sc 0.20**
- pseudo_sc は source_weight 0.5 (hard label より noisy)
- R2 student は新規初期化 (R1 の重みは流用しない、Babych 流)
- Backbone は R1 と同じ regnety_008 (6M)
...
- 入力 pseudo: `/content/drive/MyDrive/kaggle/birdclef2026/output/exp016/r1-pseudo/pseudo_labels.csv`
- 出力 R2 ckpt: `output/exp016/r2/`
- 出力 R2 pseudo: `output/exp016/r2-pseudo/pseudo_labels.csv`
...
DRIVE_R1_PSEUDO  = DRIVE_INPUT_DIR / "output" / "exp016" / "r1-pseudo" / "pseudo_labels.csv"
DRIVE_OUTPUT_DIR = DRIVE_INPUT_DIR / "output" / "exp016" / "r2"
DRIVE_R2_PSEUDO_DIR = DRIVE_INPUT_DIR / "output" / "exp016" / "r2-pseudo"
