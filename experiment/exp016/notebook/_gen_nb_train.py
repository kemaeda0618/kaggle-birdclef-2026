"""Generate exp016 NB: regnety_008 SED training on Colab Pro A100.

Design philosophy:
- **Colab Pro A100 single-session**: regnety_008 (~6M params) で 25 epoch ~3-5h、1-session 完走
- データは Drive (`/content/drive/MyDrive/kaggle/birdclef2026/`) から /content/data へ copy
- 出力 ckpt は /content/output に保存 + Drive へミラー (epoch ごと)
- exp015 R1 と同じ recipe、backbone のみ regnety_008 に差替

Architecture:
- Backbone: regnety_008 (~6M、RegNet 系、HGNet/ConvNeXt/EffNet と diversity)
- Perch v2 teacher (Google's model)
- BC2026 competition data only
- Tucker-recipe-inspired SED head (GeMFreq + bottleneck + attention)
...
# Cell 0: Header
# =============================================================================
cells.append(md_cell(r"""# exp016 — regnety_008 SED training (Colab Pro A100)

**Goal:** regnety_008 SED + Perch v2 distillation、25 epochs、Colab Pro A100 で 1 session 完走。

## なぜ regnety_008?
- 6M params、RegNet 系 (BN+SiLU+Group Conv)
- HGNet (BN+ReLU+HG-block) / ConvNeXt (LN+GELU+DW) / EffNet (BN+SiLU+MBConv) と原理的 diversity
- BC25 1位 Nikita 採用、軽量で 1-session 完走可

## Recipe (exp015 R1 と同じ、backbone のみ差替)
- focal 0.85 / labeled_sc 0.15、pseudo 無し
- Perch v2 MSE distillation (alpha=1.0)
- 25 ep / batch=64 / AdamW lr=3e-4 / cosine + warmup 2ep
- soft mixup (alpha=0.4)、SpecAugment

## Drive 構成 (memory: user_colab_drive_folder.md)
- input: `/content/drive/MyDrive/kaggle/birdclef2026/`
  - `train_audio/`, `train_soundscapes/`, `train.csv`, `taxonomy.csv`, `sample_submission.csv`, `train_soundscapes_labels.csv`
- output mirror: `/content/drive/MyDrive/kaggle/birdclef2026/output/exp016/r1/`
- Perch ONNX: Drive にあればそこから、無ければ Kaggle DL (`tuckerarrants/perch-v2-no-dft-onnx`)

## 出力
- `/content/output/ckpt_latest.pth`
- `/content/output/ckpt_ep{NN}.pth`
- `/content/output/ckpt_best_ns22.pth`、`ckpt_best_macro.pth`
- `/content/output/history.json`
- すべて Drive output dir にミラー
""", "hdr"))
...
DRIVE_OUTPUT_DIR = DRIVE_INPUT_DIR / "output" / "exp016" / "r1"
...
# kaggle.json (for Perch ONNX DL if needed)
KJ_CANDIDATES = [
...
if KJ is not None:
    ...
    print("kaggle.json not found (Perch ONNX must already be on Drive)")
...
if not PERCH_PATH.exists():
    ...
    if drive_perch_candidates:
        ...
        t0 = time.time()
        # tqdm-wrapped chunk copy for progress
        with open(src, "rb") as fin, open(PERCH_PATH, "wb") as fout:
            ...
            while True:
                buf = fin.read(8 * 1024 * 1024)  # 8MB chunks
                if not buf: break
...
# =============================================================================
# Cell 9: init (Drive resume optional)
# =============================================================================
cells.append(code_cell(r"""# ============================================================
...
for epoch in range(start_epoch, N_TOTAL_EPOCHS):
    ...
    # Mirror to Drive (latest + best + history; skip per-epoch to save Drive space/time)
...
if trained_up_to >= N_TOTAL_EPOCHS:
    ...
    print(f"\n  ... Re-run this NB to continue from epoch {trained_up_to}/{N_TOTAL_EPOCHS}")
""", "summary"))
