"""Generate exp020 R1 5-fold NB: eca_nfnet_l0, per-step Perch (no cache), TTA pseudo.

Design:
- R1 ONLY (R2 は別 NB で Perch cache を使う)
- per-step Perch ONNX 推論 (Focal の continuous random crop 維持 = exp017 R1 v2 path)
- 5-fold loop with Drive resume per fold
- R1 pseudo generation with delta-shift TTA (3-view) per fold, saved to Drive
- WARMUP_EPOCHS=3 (exp017 R1 v2 と同条件)

Estimated time (Blackwell, 5-fold):
- Per fold: R1 ~43 min + pseudo+TTA ~10 min ≈ 53 min
- 5-fold total: ~4.5h + DL 15 min ≈ ~5h
...
cells.append(md_cell(r"""# exp020 R1 5-fold (no cache, per-step Perch, TTA pseudo)

**Goal:** exp017 R1 v2 (val 0.9166) を 5-fold 化、R1 ckpt + R1 pseudo CSV を Drive に保存。
R2 学習は別 NB (`nb_train_r2_5fold.ipynb`、Perch cache 利用)。

## Key 設計
- **Perch cache 不使用** (per-step ONNX inference): Focal の連続 random crop 維持
- **WARMUP_EPOCHS=3** (exp017 R1 v2 と同条件)
- **N_TOTAL_EPOCHS=25**
...
## Drive 出力 (各 fold)
- `output/exp020/fold{k}/r1/ckpt_*.pth` (R1 ckpt)
- `output/exp020/fold{k}/r1-pseudo/pseudo_view{0..4}_raw.csv` (各 view raw probs)
- `output/exp020/fold{k}/r1-pseudo/pseudo_views_meta.json` (shift メタ)
- `output/exp020/fold{k}/r1-pseudo/pseudo_labels.csv` (5 view 平均+smooth+PT、R2 入力用)

## 想定: ~5.5h on Blackwell (R1 train 3.6h + pseudo+TTA 5 view 75 min + DL + upload)
""", "hdr"))
...
# ★ NFNet rule: LR=3e-4 据え置き、WARMUP=3 (exp017 R1 v2 と同)
N_TOTAL_EPOCHS = 25
BATCH = 192
LR = 3e-4
MIN_LR = 1e-6
...
USER  = "maekeso"
SLUG  = "birdclef2026-exp020-r1-5fold"
TITLE = "birdclef2026 exp020 R1 5fold weights"
...
with tempfile.TemporaryDirectory() as td:
    ...
    version_notes = f"exp020 R1 5fold | {summary}"[:498]
...
print(f"\n{'='*60}")
print(f"exp020 R1 5-fold summary")
print(f"{'='*60}")
