"""Generate exp020 R2 5-fold inference NB (Kaggle CPU 90min, internet=off).

Loads R2 5 fold ckpts from `maekeso/birdclef2026-exp020-weights-5fold`, rebuilds
eca_nfnet_l0 SED, runs 5-fold ensemble inference on test_soundscapes,
writes submission.csv.

R2 = pseudo distill round (val avg 0.9358 vs R1 avg 0.9212, +0.0146 gain)。

Run: python _gen_nb_infer_r2_5fold.py  ->  writes nb_infer_r2_5fold.ipynb
"""
import json
...
cells.append(md_cell(r"""# exp020 **R2** 5-fold Inference (Kaggle CPU 90min, internet=off)

5 fold ensemble inference using **R2** ckpts (eca_nfnet_l0、pseudo distill 後)。
...
- `maekeso/birdclef2026-exp020-weights-5fold` (R1 + R2 ckpts、`r2_fold{0..4}_ckpt_best_ns22.pth` を使用)
...
for ckpt_path in ckpt_files:
    ...
    onnx_path = ONNX_DIR / f"r2_fold{fold_id}.onnx"
    t0 = time.time()
    with torch.no_grad():
...
out_path = HERE / "nb_infer_r2_5fold.ipynb"
