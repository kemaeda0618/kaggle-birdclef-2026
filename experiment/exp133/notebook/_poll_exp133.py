"""Poll exp133 v2 verify until finished, then pull log + csv."""
...
REF = "maekeso/birdclef2026-exp133-verify-v2"
while True:
    try: st = json.loads(api.kernels_status(REF))
    except Exception as e: print("err",e); time.sleep(45); continue
    s = st.get("status"); print(time.strftime("%H:%M:%S"), s, flush=True)
    if s not in ("RUNNING","QUEUED"): print("FINAL:",json.dumps(st)); break
    time.sleep(45)
out = Path("experiment/exp133/notebook/_out"); out.mkdir(exist_ok=True)
