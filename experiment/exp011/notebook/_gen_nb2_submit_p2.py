"""Generate exp011 Phase 2 SUBMISSION notebook (CPU 90min inference).

= Phase 1 v2 submit + chunk_duration 20s. Same PP (file_conf_scale + adaptive dss).
- Loads best.pth (= best Val-A) from kernel_sources of `birdclef2026-exp011-train-phase2`
- 12 windows per 60-sec test file: each is a 20-sec window ending at end_time (start may be negative, zero-padded)
- target_size (256, 256) 据え置き
...
# Header
cells.append(md_cell("hdr", """# exp011 Phase 2: Submission (CPU 90min)

Phase 1 v2 submit (LB 0.854) と同じ PP を流用、**chunk_duration を 10s -> 20s** に変更。

- Weight: kernel_sources `maekeso/birdclef2026-exp011-train-phase2` (best.pth = best Val-A)
- 12 windows per file: end_time in {5, 10, ..., 60}, **20-sec context window 末尾 = end_time** (start_sec = et - 20、負ならゼロパディング)
- PP: file_confidence_scale (top_K=2, power=0.4) + adaptive δ-shift smoothing (α base=0.20)
...
CFG = dict(
    ...
    chunk_duration=20.0, target_size=(256, 256),  # <-- 20s, target_size 据え置き
...
# v2: Val-B best (Ep 14) を試す。Val-A best (Ep 6) は早期停止疑い → v1 LB 0.831
WEIGHT_FNAME = "best_val_b.pth"
WEIGHT_PATH = None
for cand in [
    Path(f"/kaggle/input/notebooks/maekeso/birdclef2026-exp011-train-phase2/weights/{WEIGHT_FNAME}"),
    Path(f"/kaggle/input/notebooks/maekeso/birdclef2026-exp011-train-phase2/{WEIGHT_FNAME}"),
    Path(f"/kaggle/input/birdclef2026-exp011-train-phase2/weights/{WEIGHT_FNAME}"),
]:
    if cand.exists():
...
# MEL TRANSFORM (identical to mel cache spec)
# ==============================================================
...
if not test_files:
    ...
    # PP-1: file_confidence_scale (NB3 v20 +0.010)
    ...
    # PP-2: adaptive delta-shift smoothing (NB3 v21)
    smoothed = view.copy()
    n_t = view.shape[1]
    alpha_means = []
