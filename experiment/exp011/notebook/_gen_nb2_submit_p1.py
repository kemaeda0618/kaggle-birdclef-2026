"""Generate exp011 Phase 1 SUBMISSION notebook (CPU 90min inference).

- Loads best.pth from kernel_sources (trained NB output)
- Reads test_soundscapes/*.ogg, computes mel on-the-fly (same params as mel cache)
- 12 windows per file (end_time=5..60), 10-sec context window ending at end_time
- Outputs submission.csv at /kaggle/working/submission.csv
...
# ── Header ──
cells.append(md_cell("hdr", """# exp011 Phase 1: Submission (CPU 90min)

- Weight: kernel_sources `maekeso/birdclef2026-exp011-train-phase1-sed` (best.pth)
- Input: `/kaggle/input/birdclef-2026/test_soundscapes/*.ogg` (60-sec each)
- Output: `/kaggle/working/submission.csv`

Mel transform は学習時 mel cache と同一仕様 (sr=32000, n_mels=256, n_fft=2048, hop=512, fmin=20, fmax=16000, power=2.0, top_db=80, db_range=[-80, 20])。

12 windows per file: end_time in {5, 10, ..., 60}, 10-sec context window 末尾 = end_time。
"""))
...
DEVICE = torch.device("cpu")  # submission is CPU only
...
# CONFIG (must match training: _gen_nb1_train_p1.py)
# ==============================================================
CFG = dict(
    sr=32_000, n_mels=256, n_fft=2048, hop_length=512,
    fmin=20, fmax=16_000, top_db=80,
    db_min=-80.0, db_max=20.0,
    chunk_duration=10.0, target_size=(256, 256),
    backbone="tf_efficientnetv2_b0", num_classes=234,
    in_channels=3, dropout=0.1, gem_p_init=3.0,
)
CFG["chunk_frames"] = int(CFG["chunk_duration"] * CFG["sr"] / CFG["hop_length"]) + 1  # 626
CFG["db_range"] = CFG["db_max"] - CFG["db_min"]
...
# Weight from kernel_sources
WEIGHT_PATH = None
for cand in [
    Path("/kaggle/input/notebooks/maekeso/birdclef2026-exp011-train-phase1-sed/weights/best.pth"),
    Path("/kaggle/input/notebooks/maekeso/birdclef2026-exp011-train-phase1-sed/best.pth"),
    Path("/kaggle/input/birdclef2026-exp011-train-phase1-sed/weights/best.pth"),
]:
    if cand.exists():
...
# ── Mel transform (matches mel cache spec) ──
cells.append(code_cell("mel-transform", r"""# ==============================================================
# MEL TRANSFORM (identical to mel_cache_256/_gen_both.py)
# ==============================================================
...
def compute_full_mel(wav_path):
    # Load ogg -> resample -> mel_db (float32, clipped to [db_min, db_max])
    ...
    return mel_db  # (n_mels, T_full) float32
...
# ── Mel-to-tensor transform (matches training MelCacheTransform) ──
...
def predict_file(wav_path):
    # Return (12, 234) probability matrix for one 60-sec file
    full_mel = compute_full_mel(wav_path)  # (n_mels, T_full)
    # Build 12 chunks: 10-sec window ending at each end_time
    chunks = []
    for et in END_TIMES:
        start_sec = et - CFG["chunk_duration"]   # may be negative
        start_frame = int(start_sec * FRAMES_PER_SEC)
        chunks.append(crop_window(full_mel, start_frame, CHUNK_FR))
    batch = torch.stack(chunks, dim=0)  # (12, n_mels, chunk_frames)
...
# ── Run inference + write submission ──
cells.append(code_cell("submission", r"""# ==============================================================
# RUN ALL FILES -> apply post-processing -> submission.csv
# ==============================================================
SUB_PATH = Path("/kaggle/working/submission.csv")

# Post-processing hyperparameters (from exp010 NB3 v20-21)
PP_FILE_CONF_TOPK = 2          # file_confidence_scale top-K
PP_FILE_CONF_POWER = 0.4       # file_confidence_scale power
PP_DSS_BASE_ALPHA = 0.20       # adaptive delta-shift smoothing base alpha
...
if not test_files:
    ...
    # 1) Predict all files into stacked (n_files, 12, 234)
    all_probs = []
    all_stems = []
    t0 = time.time()
    for i, fp in enumerate(test_files):
        probs = predict_file(fp)  # (12, 234)
    ...
    view = np.stack(all_probs, axis=0).astype(np.float32)  # (n_files, 12, n_cls)
    print(f"Predictions shape: {view.shape}")

    # 2) PP-1: file_confidence_scale (NB3 v20 +0.010 実証)
    ...
    # 3) PP-2: adaptive delta-shift smoothing (NB3 v21, alpha=0.20*(1-conf))
    smoothed = view.copy()
    n_t = view.shape[1]
    alpha_means = []
    ...
    # 4) Build submission DataFrame (vectorized)
    flat = view.reshape(-1, view.shape[-1])  # (n_files*12, n_cls)
...
out_path = HERE / "nb2_submit_p1.ipynb"
