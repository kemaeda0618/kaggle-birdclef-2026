"""Generate exp100: exp017 R2 single teacher → l1 student (single fold) NB.
...
cells.append(md_cell(r"""# exp100 R3 (l1 single fold, exp017 R2 teacher)
...
- 最終: Kaggle Dataset `birdclef2026-exp100-l1-babych` に R3 ckpt upload
...
DRIVE_EXP_DIR    = DRIVE_INPUT_DIR / "output" / "exp100"
...
# ★ exp100: teacher pseudo (pseudo_e17.csv) を Kaggle NB output から DL → Drive へ
...
BACKBONE = "eca_nfnet_l1"   # ★ exp100: l0 → l1 (capacity 増、Option B 検証)
USE_PERCH_DISTILL = True
PERCH_EMBED_DIM = 1536
ALPHA_DISTILL = 1.0

N_FOLDS = 5               # ★ exp100: 5-fold split で分割するが、train するのは fold 0 のみ
...
USE_MIXUP = True
MIXUP_PROB = 0.5
MIXUP_ALPHA = 1.0  # ★ exp100: 0.4 -> 1.0 (Babych BC25 1st spec)
MIXUP_HARD = False
FREQ_MASK_PARAM = 25
TIME_MASK_PARAM = 30
NUM_FREQ_MASKS = 2
...
SHARES_R2 = {"focal": 0.60, "labeled_sc": 0.10, "pseudo_sc": 0.30}   # ★ exp100 Option C: pseudo 0.20 → 0.25、distill 強化
...
class BirdSEDModel(nn.Module):
    def __init__(self, backbone_name=BACKBONE, num_classes=NUM_CLASSES,
                 drop_path_rate=0.15, hidden_dim=512):  # ★ exp100: 0.1 -> 0.15 (Babych R2+ spec)
        super().__init__()
...
def train_r2(fold_k, pseudo_meta_df, Y_pseudo, out_dir):
    ...
    for epoch in range(N_TOTAL_EPOCHS):
        ...
        smp = MixSamp(SIZES, NAMES, SHARES_R2, BATCH, n_steps_ep, seed=42 + epoch, focal_weights=_focal_weights)
...
for FOLD_K in FOLDS:
    print(f"\n{'#'*70}\n# exp100 R3 (l1 student, e17 teacher) Fold {FOLD_K} (of {N_FOLDS}-fold split, single fold training)\n{'#'*70}")
    DRIVE_FOLD_DIR  = DRIVE_EXP_DIR / f"fold{FOLD_K}"
    # ★ exp100: teacher pseudo は exp017 R2 (pseudo_e17.csv) を Drive 上に置く前提
...
USER  = "maekeso"
SLUG  = "birdclef2026-exp100-l1-babych"
TITLE = "birdclef2026 exp100 l1 babych (R3 from e17, mixup 1.0 + dp 0.15 + WRS)"

with tempfile.TemporaryDirectory() as td:
    td = Path(td)
    n_staged = 0
    for fold_k in FOLDS:
        # ★ exp100: R3 ckpt のみ upload (R1/R2 は別 exp で管理)
    ...
    version_notes = f"exp100 l1 single (R3 from e17) | {summary}"[:498]
...
print(f"\n{'='*60}")
print(f"exp100 l1 single fold R3 summary")
print(f"{'='*60}")
...
for k, v in sorted(fold_results.items()):
    print(f"  Fold {k}: {v}")
print(f"\n  Kaggle Dataset: https://www.kaggle.com/datasets/maekeso/birdclef2026-exp100-l1-babych")
""", "summary"))
