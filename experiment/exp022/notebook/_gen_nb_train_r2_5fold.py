"""Generate exp022 R2 5-fold NB: eca_nfnet_l1 + Perch cache + R1 pseudo from Drive.

Design:
- exp020 派生、backbone のみ l0 → l1 (~41M params、1.7x l0)
- R2 ONLY (R1 ckpt + R1 pseudo は別 NB で先に作成済み前提)
- Perch cache 利用 (Focal augmentation 影響薄、速度優先)
- R1 pseudo を Drive から fold ごとに読込
- R2 pseudo gen 不要 (R3 やらない)
- N_TOTAL_EPOCHS=**15** (Plan I: peak ep 4-8 に +7-11 ep buffer、best_ckpt 自動保存で peak 確実捕捉)
- NUM_WORKERS=16 (data loading 並列度 +33%)
- WARMUP=**4** (l1 で +1 ep、l0 の 3 から発散リスク緩和)

Estimated: ~10-12h on Blackwell 96GB (l0 比 ~1.7x)

Run: python _gen_nb_train_r2_5fold.py
"""
import json
...
cells.append(md_cell(r"""# exp022 R2 5-fold (eca_nfnet_l1, Perch cache, R1 pseudo from Drive)

**前提**: `nb_train_r1_5fold.ipynb` (exp022) 完走済、Drive に R1 ckpt + R1 pseudo CSV あり。

## 構成 (vs exp020)
- **BACKBONE = `eca_nfnet_l1`** (~41M params、1.7x l0)
- **WARMUP_EPOCHS = 4** (l0 の 3 から +1 ep、l1 の発散リスク緩和)
- Perch cache 使用 (`maekeso/birdclef2026-perch-emb-cache`)
- R1 pseudo from Drive (`output/exp022/fold{k}/r1-pseudo/pseudo_labels.csv`)
- **N_EPOCHS=15** (Plan I、peak ep 4-8 に buffer +7-11、best_ckpt で確実 peak 捕捉)、batch=192、LR=3e-4
- NUM_WORKERS=16 (data loading 並列度 +33%)
- 5-fold loop、Drive resume per fold
- R2 pseudo gen は skip
- 最終: Kaggle Dataset `birdclef2026-exp022-weights-5fold` に R1 + R2 ckpt まとめて upload

## 想定: ~10-12h on Blackwell 96GB (l0 比 ~1.7x、Plan I で更に短縮済)
""", "hdr"))
...
BACKBONE = "eca_nfnet_l1"   # ★ exp022: l0 → l1、~41M params、1.7x l0
...
# l1 で 5-fold ~10-12h (l0 比 ~1.7x)、Plan B (multi-teacher) は別途
N_TOTAL_EPOCHS = 15
BATCH = 192
LR = 3e-4
MIN_LR = 1e-6
...
for FOLD_K in FOLDS:
    print(f"\n{'#'*70}\n# R2 Fold {FOLD_K} / {N_FOLDS-1}\n{'#'*70}")
    DRIVE_FOLD_DIR  = DRIVE_EXP_DIR / f"fold{FOLD_K}"
    DRIVE_R1_PSEUDO = DRIVE_FOLD_DIR / "r1-pseudo" / "pseudo_labels.csv"
    DRIVE_R2_DIR    = DRIVE_FOLD_DIR / "r2"
    DRIVE_R2_DIR.mkdir(parents=True, exist_ok=True)

    if not DRIVE_R1_PSEUDO.exists():
        print(f"  [SKIP] R1 pseudo missing: {DRIVE_R1_PSEUDO}")
        print(f"  Run nb_train_r1_5fold.ipynb first to generate R1 pseudo")
        fold_results[FOLD_K] = {"status": "skipped (no R1 pseudo)"}
        continue

    R2_BEST = DRIVE_R2_DIR / "ckpt_best_ns22.pth"
    if R2_BEST.exists():
        print(f"  Fold {FOLD_K}: R2 ckpt already on Drive — skip")
        try:
    ...
    print(f"  Loading R1 pseudo: {DRIVE_R1_PSEUDO}")
    pseudo_df_r1 = pd.read_csv(DRIVE_R1_PSEUDO)
...
USER  = "maekeso"
SLUG  = "birdclef2026-exp022-weights-5fold"
TITLE = "birdclef2026 exp022 weights 5fold"
...
with tempfile.TemporaryDirectory() as td:
    ...
    summary = " ; ".join([f"f{k}_r2={v.get('r2_best_ns22','?')}" for k, v in sorted(fold_results.items())])
    version_notes = f"exp022 5fold (l1, R1+R2) | {summary}"[:498]
...
print(f"\n{'='*60}")
print(f"exp022 R2 5-fold summary (eca_nfnet_l1)")
print(f"{'='*60}")
...
for k, v in sorted(fold_results.items()):
    print(f"  Fold {k}: {v}")
print(f"\n  Kaggle Dataset: https://www.kaggle.com/datasets/maekeso/birdclef2026-exp022-weights-5fold")
""", "summary"))
...
out_path = HERE / "nb_train_r2_5fold.ipynb"
