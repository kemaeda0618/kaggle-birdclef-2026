"""Generate exp022 R1 5-fold inference NB (Kaggle CPU 90min, internet=off).

Loads R1 5 fold ckpts from `maekeso/birdclef2026-exp022-r1-5fold`, rebuilds
eca_nfnet_l1 SED, runs 5-fold ensemble inference on test_soundscapes,
writes submission.csv.

5 model forward per file × 700 files = ~75-100 min CPU (l1 で l0 比 ~1.3x、90 min 制限注意)。
...
cells.append(md_cell(r"""# exp022 R1 5-fold Inference (Kaggle CPU 90min, internet=off)

5 fold ensemble inference using R1 ckpts (eca_nfnet_l1)。
Predictions are averaged across 5 fold models, then Gaussian smoothed + sigmoid'd.

## Inputs
- `birdclef-2026` (competition data)
- `maekeso/birdclef2026-exp022-r1-5fold` (5 fold R1 ckpts、r1_fold{0..4}_ckpt_best_ns22.pth)

## Time budget
- ~75-100 min on Kaggle CPU (5 models × 700 files × 12 windows、l1 で l0 比 ~1.3x)
- **90 min 制限注意**、必要なら ensemble model 数を絞る or chunk 並列化
...
STATE_DIR = None
CANDIDATES = [
    Path("/kaggle/input/datasets/maekeso/birdclef2026-exp022-r1-5fold"),
    Path("/kaggle/input/birdclef2026-exp022-r1-5fold"),
]
for p in CANDIDATES:
    if p.exists():
...
assert STATE_DIR is not None, (
    "ckpts not found. Attach maekeso/birdclef2026-exp022-r1-5fold as dataset_sources"
)
