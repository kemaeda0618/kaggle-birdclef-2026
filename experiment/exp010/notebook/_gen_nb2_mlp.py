"""Generate exp010 NB2 v10: MLP Head Submission Notebook (NB3 v29 同構造).

Train MLP Head on Perch embeddings (from NB1) with full NB3 v29 infra:
- multi-seed=5 ensemble
- site/hour metadata embedding
- TTA shifts [-1, 0, 1]
- Prior Tables (site/hour/sh shrinkage, lambda=0.3)
- file_confidence_scale (top_k=2, power=0.4)
- Hierarchical Site-Conditioned KNN retrieval (lambda=0.10)

Pipeline (NB3 v29 と同構造):
1. Load pre-computed embeddings (train_soundscapes)
2. Train MLP Head (multi-seed) on labeled soundscape windows (66 files) with Gated Fusion
3. Run Perch ONNX on test_soundscapes (CPU)
4. MLP inference + retrieval blend -> submission.csv

Usage: python _gen_nb2_mlp.py
"""
import json
...
cells.append(md_cell("hdr",
    "# exp010 NB2 v10: MLP Head on Perch Embeddings (NB3 v29 同構造)\n"
    "\n"
    "CPU submission notebook (90 min limit). NB3 v29 infra を MLP Head に適用。\n"
    "1. Load pre-computed Perch embeddings from NB1\n"
    "2. Train MLP Head (multi-seed=5) on labeled soundscapes (66 files)\n"
    "3. Perch ONNX inference on test_soundscapes (CPU)\n"
    "4. MLP TTA + retrieval blend -> submission.csv\n"
    "\n"
    "Differences vs NB3 v29: ProtoSSM → MLP Head (per-window, no cross-window seq processing)"))
...
# Hierarchical Site-Conditioned KNN retrieval (inference-only post-hoc logit blend)
RETRIEVAL_K = 10
RETRIEVAL_TAU = 0.05
...
# RETRIEVAL POOL (per-window labeled embeddings + metadata) for inference-time KNN
...
# ── MLP Head Model (with site/hour emb + prior_logit) ──
MODEL = """\
# MLP HEAD MODEL (NB3 v29 同構造、ProtoSSM を Per-window MLP に置換)
class MLPHead(nn.Module):
    '''Per-window MLP with site/hour metadata + Gated Fusion + prior_logit support.'''
    def __init__(self, d_input, d_hidden, n_classes, dropout=0.1,
        ...
        # Gated fusion with Perch logits (per-class)
...
print("MLP Head model defined.")"""
cells.append(code_cell("model", MODEL))

# ── Train MLP Head (multi-seed) ──
TRAIN = """\
# TRAIN MLP HEAD (multi-seed ensemble, NB3 v29 同手順)
...
for si, seed in enumerate(SEEDS):
    ...
    if si == 0:
        print(f"Training MLP Head (x{len(SEEDS)} seeds): {sum(p.numel() for p in model.parameters())} params/model")
    t0 = time.time()
...
print(f"Ensemble: {len(models)} MLP models trained in {time.time()-t0_all:.1f}s")"""
...
# Hierarchical Site-Conditioned KNN retrieval logit (NB3 v29 同関数)
...
# ── Inference (TTA + multi-seed + meta + prior + retrieval) ──
INFERENCE = """\
# INFERENCE ON TEST SOUNDSCAPES (NB3 v29 同手順)
...
for fi, fpath in enumerate(test_files):
    ...
    with torch.no_grad():
        ...
        # Hierarchical Site-Conditioned KNN retrieval blend
...
# ── Submission (with file_confidence_scale) ──
SUBMISSION = """\
# BUILD SUBMISSION (with file_confidence_scale, NB3 v29 同コード)
preds_array = np.stack(all_probs)

# file_confidence_scale (top-K mean ^ power)
FCS_TOP_K = 2
FCS_POWER = 0.4
