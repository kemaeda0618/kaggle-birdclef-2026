# Title: Sharing baseline LB.928. Aiming to achieve both reproducibility and inference speed[Fixed(～2026/04/06)]
...
* Randomness in Data Augmentation (Mixup / CutMix).
In the `mixup_cutmix` and `mixup_files` functions, the NumPy random number generator is used without a fixed seed.
* Random Masking in the Dropout Layer
The numerous nn.Dropout(dropout) methods placed within the network drop neurons by relying on PyTorch's global random number generator. If the seed is not fixed, the areas that are masked will change with each execution.
...
* Batching of TTA ("temporal_shift_tta")
Current bottleneck: Reducing the process within the temporal_shift_tta function, which loops through the shift amount (5 patterns) and calls the model inference (model(...)) 5 times separately.
...
P.S. I was concerned about a timeout issue, so I tried submitting twice, and both times the scoring completed successfully.
However, the inference time seems to be just barely enough, so I think there's a possibility of a timeout under congested conditions. Therefore, I plan to update the implementation when I have more time.
...
While the model is performing inference, a separate thread reads the audio file from disk in parallel and keeps it waiting in memory.
This allows disk I/O and CPU operations to fully overlap (operate in parallel), reducing the total processing time.
