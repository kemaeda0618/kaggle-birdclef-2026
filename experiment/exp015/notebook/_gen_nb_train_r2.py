"""Generate exp015 R2 NB: convnext_pico.d1_in1k SED + R1 pseudo (Colab Pro A100).

Round 2 of Multi-Iterative Noisy Student:
- R1 student (exp015 convnext_pico.d1_in1k、val_ns22 0.9221) が生成した pseudo を 3rd source として使用
- R2 student も同 backbone (convnext_pico.d1_in1k)、新規初期化 (Babych 流)
- 学習 → pseudo R2 生成 → 重み Kaggle Dataset upload まで 1 ipynb で完結
...
入出力:
- 入力: Drive `output/exp015/r1-pseudo/pseudo_labels.csv` (R1 pseudo)
        Drive train_audio / train_soundscapes (Kaggle API 直 DL 経由)
- 出力 ckpt: Drive `output/exp015/r2/`
- 出力 pseudo: Drive `output/exp015/r2-pseudo/pseudo_labels.csv`
- Kaggle Dataset: `maekeso/birdclef2026-exp015-weights` (version up、R2 重み追加)
...
# Header
# =============================================================================
cells.append(md_cell(r"""# exp015 R2 — convnext_pico.d1_in1k SED + R1 pseudo (Colab Pro A100)

**Round 2 of Multi-Iterative Noisy Student** (Babych BC2025 1位レシピ準拠)。

## What's new in R2
- R1 student (val_ns22 0.9221) が生成した pseudo CSV を 3rd training source として使用
- Source mix: **focal 0.70 / labeled_sc 0.10 / pseudo_sc 0.20**
- pseudo_sc は source_weight 0.5 (hard label より noisy)
- R2 student は新規初期化 (R1 の重みは流用しない、Babych 流)
- Backbone は R1 と同じ convnext_pico.d1_in1k (9M、LN+GELU+DW)

## 1 ipynb 統合構成 (feedback_train_pseudo_same_ipynb)
1-13: train phase (R2 学習)
14-17: pseudo phase (R2 pseudo CSV 生成)
18:   upload phase (重み Kaggle Dataset 化)

## Drive 構成
- 入力 ckpt source: 不要 (R2 は新規初期化)
- 入力 pseudo: `/content/drive/MyDrive/kaggle/birdclef2026/output/exp015/r1-pseudo/pseudo_labels.csv`
- 出力 R2 ckpt: `output/exp015/r2/`
- 出力 R2 pseudo: `output/exp015/r2-pseudo/pseudo_labels.csv`

## 想定時間 (Colab Pro A100)
- 学習 25 epoch: ~3-3.5h (pseudo_sc 追加で R1 比 +20-30%)
- pseudo R2 生成: 20-40 min
- 重み upload: 1-2 min
- **合計: ~3.5-4.5h**

## 期待
- val_ns22 0.93-0.935 (R1 から +0.01-0.015)
- LB 推定 ~0.915 (val→LB gap -0.018)
""", "hdr"))
...
DRIVE_R1_PSEUDO  = DRIVE_INPUT_DIR / "output" / "exp015" / "r1-pseudo" / "pseudo_labels.csv"
DRIVE_OUTPUT_DIR = DRIVE_INPUT_DIR / "output" / "exp015" / "r2"
DRIVE_R2_PSEUDO_DIR = DRIVE_INPUT_DIR / "output" / "exp015" / "r2-pseudo"
...
# R1 pseudo は Drive 既存 or Kaggle Dataset から取得 (Cell 2 で fallback DL)
HAS_DRIVE_PSEUDO = DRIVE_R1_PSEUDO.exists()
print(f"Drive input:    {DRIVE_INPUT_DIR}")
if HAS_DRIVE_PSEUDO:
    print(f"R1 pseudo CSV (Drive):  {DRIVE_R1_PSEUDO} ({DRIVE_R1_PSEUDO.stat().st_size/1e6:.1f}MB)")
else:
    print(f"R1 pseudo CSV (Drive): 未取得 → Cell 2 で Kaggle Dataset (birdclef2026-exp015-pseudo-r1) から DL")
...
# =============================================================================
# Cell 2: Data prep — Kaggle API direct DL + R1 pseudo copy
# =============================================================================
cells.append(code_cell(r"""# ============================================================
...
# R1 pseudo CSV (Drive 既存優先 → fallback で Kaggle Dataset DL)
...
if not LOCAL_PSEUDO_CSV.exists():
    if HAS_DRIVE_PSEUDO:
        ...
        print("\nDownloading R1 pseudo from Kaggle Dataset (maekeso/birdclef2026-exp015-pseudo-r1)...")
        api.dataset_download_files(
            "maekeso/birdclef2026-exp015-pseudo-r1",
            path=str(LOCAL_PSEUDO_DIR), unzip=True, quiet=False,
        )
        if not LOCAL_PSEUDO_CSV.exists():
            hits = list(LOCAL_PSEUDO_DIR.rglob("pseudo_labels.csv"))
            assert hits, "pseudo_labels.csv not found after DL"
            if hits[0] != LOCAL_PSEUDO_CSV:
                shutil.move(str(hits[0]), str(LOCAL_PSEUDO_CSV))
        # Drive にもキャッシュしておく (次回以降速い)
        DRIVE_R1_PSEUDO.parent.mkdir(parents=True, exist_ok=True)
        if not DRIVE_R1_PSEUDO.exists():
            print(f"  Caching to Drive: {DRIVE_R1_PSEUDO}")
            shutil.copy2(str(LOCAL_PSEUDO_CSV), str(DRIVE_R1_PSEUDO))
...
N_FOLDS = 5
FOLDS = [0]
N_TOTAL_EPOCHS = 20    # memory rule: default-epochs-20、best ckpt 自動保存で peak (ep4-10) 取得
# ★ R2 on Blackwell: VRAM 96GB を活かして batch を倍に、LR は sqrt(2) で補正、WARMUP +1ep
BATCH = 128
LR = 4.2e-4    # 3e-4 × sqrt(2) ≈ 4.24e-4
MIN_LR = 1e-6
WD = 1e-4
WARMUP_EPOCHS = 3
...
# ★ R2: 3-source mix (Babych BC25 1位 recipe)
...
# =============================================================================
# Cell 4: Load data (incl. R1 pseudo)
# =============================================================================
cells.append(code_cell(r"""# ============================================================
...
# =============================================================================
cells.append(md_cell(r"""## ─────── R2 Pseudo Phase ───────
...
# =============================================================================
# Cell 18: Kaggle Dataset upload (R2 weights)
# =============================================================================
cells.append(code_cell(r"""# ============================================================
# Cell 18: Upload R2 weights to Kaggle Dataset (version up existing slug)
# ============================================================
import tempfile
from kaggle.api.kaggle_api_extended import KaggleApi
...
USER  = "maekeso"
SLUG  = "birdclef2026-exp015-weights"
TITLE = "birdclef2026 exp015 weights"

# R2 ckpts to upload (R2 best ns22 / macro / latest + history)
# 注: 同じ Dataset slug に R2 ckpt も追加する (version up)
TARGETS = [
...
with tempfile.TemporaryDirectory() as td:
    ...
    # Pull existing R1 files from the previous Dataset version into the new version
    # (otherwise Kaggle replaces — version up = full replacement)
    print(f"\nFetching R1 files from existing Dataset version to preserve them...")
    try:
        ...
        for f in tmp_dl.iterdir():
            if f.is_file() and not f.name.startswith("r2_") and not (td / f.name).exists():
                shutil.copy2(str(f), str(td / f.name))
                print(f"  preserved R1: {f.name}")
        shutil.rmtree(tmp_dl)
    except Exception as e:
        print(f"  [WARN] could not preserve R1 files: {str(e)[:200]}")
        print(f"  R2 will be uploaded as a fresh version (R1 files may be replaced)")
    ...
    version_notes = f"R2 best_ns22={best_ns22:.4f} (R1 was 0.9207)"
    uploaded = False
...
# =============================================================================
# Cell N+1: Colab runtime disconnect — compute units の節約
# =============================================================================
cells.append(code_cell(r"""# ============================================================
# Cell: Disconnect Colab runtime (compute units 節約)
# ============================================================
# 学習・pseudo 生成・upload が全て完了したらここで runtime を切断する。
# Colab Pro の compute units はアイドル時間にも消費されるため、最後まで実行できたら
# 明示的に runtime.unassign() で切る。
#
# 注意:
# - これより上の cell が全て成功している前提で実行 (assert で失敗していたらここまで来ない)
# - 上の cell に手動で `Stop` を押した場合はここに到達しないので、その時は手動切断する
# - 5 秒だけ wait して、ログを確実に flush してから切断
import time
print("\n" + "="*60)
print(f"All cells completed at {time.strftime('%Y-%m-%d %H:%M:%S')}")
print(f"Disconnecting runtime in 5s to save Colab compute units...")
print("="*60)
time.sleep(5)
try:
    ...
    print("Manually disconnect via Runtime → Disconnect runtime")
""", "disconnect"))
