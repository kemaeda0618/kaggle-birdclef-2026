...
print("=== exp082-weights Dataset files ===")
try:
    files = api.dataset_list_files("maekeso/birdclef2026-exp082-weights")
...
# Peek at r2_history.json if exists
print("\n=== Peek r2 history (val metrics) ===")
try:
    with tempfile.TemporaryDirectory() as td:
        # Try downloading r2_history.json
        for cand in ["r2_history.json", "r2/history.json", "history.json"]:
            try:
                api.dataset_download_file("maekeso/birdclef2026-exp082-weights", cand,
                                          path=td, force=True)
                ...
                if p.exists():
                    ...
                    if isinstance(hist, list) and hist:
                        ...
                        last = hist[-1]
                        print(f"    Last ep {last.get('epoch')}: val_ns22={last.get('val_ns22'):.4f} taxon={last.get('val_per_taxon')}")
                        break
            except Exception:
                continue
except Exception as e:
